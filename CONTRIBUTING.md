...
- 届数目录：`{序号}-第{N}届/`，序号从 `01` 开始，`{N}` 为中文数字，如 `05-第十六届/`
- 下级分类：自第十五届起按 `省赛/`、`国赛/`、`模拟题/` 分类；第十二至十四届仅收录省赛（旧国赛题目依赖拓展板，新开发板不再适用），赛题文件直接置于届目录下
...
```
省赛/
├── {N}届省赛程序设计题.pdf              # 赛题 PDF
├── {N}届省赛客观题.pdf                  # 客观题 PDF（如有）
└── 第{中文数字}届省赛真题参考代码.zip    # 完整 MDK 工程压缩包
```

> PDF 文件名中的 `{N}` 为阿拉伯数字，如 `16`；目录名与压缩包文件名中的届数为中文数字，如 `十六`。
...
- 本项目采用 [CC BY-NC-SA 4.0](https://creativecommons.org/licenses/by-nc-sa/4.0/) 许可协议开源。提交贡献即表示你同意将内容以同一许可协议发布
